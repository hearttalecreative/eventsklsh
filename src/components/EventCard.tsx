...
export const EventCard = ({ event }: Props) => {
  ...
  return (
    <Card className="h-full flex flex-col min-w-0 overflow-hidden transition-all duration-500 hover:shadow-horizon-lg hover:-translate-y-2 group cursor-pointer">
      <CardHeader className="p-0 min-w-0 relative">
        <Link
          to={`/event/${slugPath}`}
          className="block relative w-full overflow-hidden rounded-t-2xl outline-none focus-visible:ring-4 focus-visible:ring-primary/50"
        ...
        >
          <AspectRatio ratio={1230 / 693}>
            ...
            />
            {/* Horizon-style gradient overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/30 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
          </AspectRatio>
          {(isSoldOut || isPaused) && (
            <div className="absolute top-3 right-3 z-10">
              <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide uppercase bg-black/40 backdrop-blur-md text-white shadow-sm border border-white/20">
        ...
        <div className="p-5 sm:p-6 min-w-0 pb-2">
          <CardTitle className="text-xl sm:text-2xl font-semibold leading-tight break-words transition-all duration-300 group-hover:text-gradient">
      ...
      <CardContent className="px-5 sm:px-6 pb-5 sm:pb-6 mt-auto overflow-hidden flex flex-col gap-4">
        <div className="space-y-2.5 text-sm sm:text-base pt-2">
          <div className="flex items-start justify-between gap-3">
            <span className="text-muted-foreground/70 font-medium shrink-0 uppercase tracking-wider text-xs">When</span>
          ...
          <div className="flex items-start justify-between gap-3">
            <span className="text-muted-foreground/70 font-medium shrink-0 uppercase tracking-wider text-xs">Venue</span>
          ...
          {hasTickets ? (
            <div className="flex items-center justify-between gap-3 pt-1">
              <span className="text-muted-foreground/70 font-medium shrink-0 uppercase tracking-wider text-xs">From</span>
            ...
            <div className="flex items-center justify-between gap-3 pt-1">
              <span className="text-muted-foreground/70 font-medium shrink-0 uppercase tracking-wider text-xs">Tickets</span>
