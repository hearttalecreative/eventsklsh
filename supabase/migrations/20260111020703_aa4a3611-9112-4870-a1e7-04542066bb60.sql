-- Create function to get detailed dashboard analytics with ticket and addon revenue
...
BEGIN
  ...
  revenue_by_type AS (
    SELECT 
      ...
      COALESCE(SUM(
        CASE WHEN oi.ticket_id IS NOT NULL THEN oi.total_amount_cents ELSE 0 END
      ), 0)::bigint as ticket_revenue,
      COALESCE(SUM(
        CASE WHEN oi.addon_id IS NOT NULL THEN oi.total_amount_cents ELSE 0 END
      ), 0)::bigint as addon_revenue
